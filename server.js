...
async function processarClienteExistente(phone, message, cliente) {
    ...
    // ============================================================
    // 1. VERIFICAR SE É UM COMANDO ESPECÍFICO
    // ============================================================
    
    // Se for "menu" ou "0", mostra o menu principal
    if (msg === 'menu' || msg === '0') {
...
Digite o número da opção (1-7)`;
    ...
    // Se for uma intenção específica, processa normalmente
    if (intent !== 'desconhecida') {
        const resposta = gerarRespostaBot(intent, cliente.nome, cliente.etapa_atual);
        await enviarWhatsApp(phone, resposta);
        return;
    }
    
    // ============================================================
    // 3. MOSTRAR STATUS DO PROCESSO + OFERECER AJUDA
    // ============================================================
